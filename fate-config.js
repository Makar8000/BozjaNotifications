// Bozja
// Southern Front: https://xivapi.com/search?indexes=Fate&filters=ID%3E=1597,ID%3C=1628&columns=Description,Name,Url

const fateConfig = {
  options: {
    tts: 'solo',
    push: '',
    types: ['all', 'crit', 'solo'],
    pushConfig: {
      token: '<token>',
      user: '<user>',
      device: undefined,
    },
  },
  fates: [
    {
      id: 1597,
      name: 'Sneak & Spell',
      spawnsCrit: false,
      spawnsSolo: false
    },
    {
      id: 1598,
      name: 'None of Them Knew They Were Robots',
      spawnsCrit: false,
      spawnsSolo: false
    },
    {
      id: 1599,
      name: 'The Beasts Must Die',
      spawnsCrit: false,
      spawnsSolo: false
    },
    {
      id: 1600,
      name: 'Unrest for the Wicked',
      spawnsCrit: false,
      spawnsSolo: false
    },
    {
      id: 1601,
      name: 'More Machine Now than Man',
      spawnsCrit: true,
      spawnsSolo: true
    },
    {
      id: 1602,
      name: 'Can Carnivorous Plants Bloom Even on a Battlefield?',
      spawnsCrit: false,
      spawnsSolo: false
    },
    {
      id: 1603,
      name: 'Seeq and Destroy',
      spawnsCrit: false,
      spawnsSolo: false
    },
    {
      id: 1604,
      name: 'All Pets Are Off',
      spawnsCrit: true,
      spawnsSolo: false
    },
    {
      id: 1605,
      name: 'Conflicting with the First Law',
      spawnsCrit: false,
      spawnsSolo: false
    },
    {
      id: 1606,
      name: 'Brought to Heal',
      spawnsCrit: false,
      spawnsSolo: false
    },
    {
      id: 1607,
      name: 'The Monster Mash',
      spawnsCrit: false,
      spawnsSolo: false
    },
    {
      id: 1608,
      name: 'Red (Chocobo) Alert',
      spawnsCrit: true,
      spawnsSolo: true
    },
    {
      id: 1609,
      name: 'Unicorn Flakes',
      spawnsCrit: true,
      spawnsSolo: false
    },
    {
      id: 1610,
      name: 'Parts and Recreation',
      spawnsCrit: false,
      spawnsSolo: false
    },
    {
      id: 1611,
      name: 'The Element of Supplies',
      spawnsCrit: false,
      spawnsSolo: false
    },
    {
      id: 1612,
      name: 'Heavy Boots of Lead',
      spawnsCrit: false,
      spawnsSolo: false
    },
    {
      id: 1613,
      name: 'No Camping Allowed',
      spawnsCrit: false,
      spawnsSolo: false
    },
    {
      id: 1614,
      name: 'Scavengers of Human Sorrow',
      spawnsCrit: false,
      spawnsSolo: false
    },
    {
      id: 1615,
      name: 'Help Wanted',
      spawnsCrit: false,
      spawnsSolo: false
    },
    {
      id: 1616,
      name: 'Pyromancer Supreme',
      spawnsCrit: false,
      spawnsSolo: false
    },
    {
      id: 1617,
      name: 'Waste the Rainbow',
      spawnsCrit: false,
      spawnsSolo: false
    },
    {
      id: 1618,
      name: 'The Wild Bunch',
      spawnsCrit: false,
      spawnsSolo: false
    },
    {
      id: 1619,
      name: 'My Family and Other Animals',
      spawnsCrit: false,
      spawnsSolo: false
    },
    {
      id: 1620,
      name: 'I\'m a Mechanical Man',
      spawnsCrit: true,
      spawnsSolo: false
    },
    {
      id: 1621,
      name: 'Murder Death Kill',
      spawnsCrit: false,
      spawnsSolo: false
    },
    {
      id: 1622,
      name: 'Desperately Seeking Something',
      spawnsCrit: false,
      spawnsSolo: false
    },
    {
      id: 1623,
      name: 'Supplies Party',
      spawnsCrit: false,
      spawnsSolo: false
    },
    {
      id: 1624,
      name: 'Demonstrably Demonic',
      spawnsCrit: false,
      spawnsSolo: false
    },
    {
      id: 1625,
      name: 'For Absent Friends',
      spawnsCrit: true,
      spawnsSolo: true
    },
    {
      id: 1626,
      name: 'Of Steel and Flame',
      spawnsCrit: true,
      spawnsSolo: true
    },
    {
      id: 1627,
      name: 'Let Slip the Dogs of War',
      spawnsCrit: false,
      spawnsSolo: false
    },
    {
      id: 1628,
      name: 'The War Against the Machines',
      spawnsCrit: false,
      spawnsSolo: false
    },
    {
      id: 1717,
      name: 'Of Beasts and Braggadocio',
      spawnsCrit: false,
      spawnsSolo: false
    },
    {
      id: 1718,
      name: 'Parts and Parcel',
      spawnsCrit: false,
      spawnsSolo: false
    },
    {
      id: 1719,
      name: 'An Immoral Dilemma',
      spawnsCrit: false,
      spawnsSolo: false
    },
    {
      id: 1720,
      name: 'Deadly Divination',
      spawnsCrit: false,
      spawnsSolo: false
    },
    {
      id: 1721,
      name: 'A Wrench in the Reconnaissance Effort',
      spawnsCrit: false,
      spawnsSolo: false
    },
    {
      id: 1722,
      name: 'Another Pilot Episode',
      spawnsCrit: true,
      spawnsSolo: true
    },
    {
      id: 1723,
      name: 'Breaking the Ice',
      spawnsCrit: false,
      spawnsSolo: false
    },
    {
      id: 1724,
      name: 'Meet the Puppetmaster',
      spawnsCrit: false,
      spawnsSolo: false
    },
    {
      id: 1725,
      name: 'Challenge Accepted',
      spawnsCrit: false,
      spawnsSolo: false
    },
    {
      id: 1726,
      name: 'Th\'uban the Terrible',
      spawnsCrit: false,
      spawnsSolo: false
    },
    {
      id: 1727,
      name: 'An End to Atrocities',
      spawnsCrit: false,
      spawnsSolo: false
    },
    {
      id: 1728,
      name: 'A Just Pursuit',
      spawnsCrit: false,
      spawnsSolo: false
    },
    {
      id: 1729,
      name: 'Tanking Up',
      spawnsCrit: false,
      spawnsSolo: false
    },
    {
      id: 1730,
      name: 'Supersoldier Rising',
      spawnsCrit: false,
      spawnsSolo: false
    },
    {
      id: 1731,
      name: 'Demented Mentor',
      spawnsCrit: false,
      spawnsSolo: false
    },
    {
      id: 1732,
      name: 'Sever the Strings',
      spawnsCrit: false,
      spawnsSolo: false
    },
    {
      id: 1733,
      name: 'The Beasts Are Back',
      spawnsCrit: false,
      spawnsSolo: false
    },
    {
      id: 1734,
      name: 'Still Only Counts as One',
      spawnsCrit: false,
      spawnsSolo: false
    },
    {
      id: 1735,
      name: 'Seeq and You Will Find',
      spawnsCrit: false,
      spawnsSolo: false
    },
    {
      id: 1736,
      name: 'Mean-spirited',
      spawnsCrit: false,
      spawnsSolo: false
    },
    {
      id: 1737,
      name: 'A Relic Unleashed',
      spawnsCrit: false,
      spawnsSolo: false
    },
    {
      id: 1738,
      name: 'When Mages Rage',
      spawnsCrit: false,
      spawnsSolo: false
    },
    {
      id: 1739,
      name: 'Hypertuned Havoc',
      spawnsCrit: false,
      spawnsSolo: false
    },
    {
      id: 1740,
      name: 'Attack of the Supersoldiers',
      spawnsCrit: false,
      spawnsSolo: false
    },
    {
      id: 1741,
      name: 'The Student Becalms the Master',
      spawnsCrit: false,
      spawnsSolo: false
    },
    {
      id: 1742,
      name: 'Attack of the Machines',
      spawnsCrit: false,
      spawnsSolo: false
    },
  ]
}